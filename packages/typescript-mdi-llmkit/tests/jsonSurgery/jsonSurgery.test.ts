          ...
          schemaDescription: `
{
  ...
  `,
    ...
    it('supports order-sensitive array edits for checklist-style prompts', async () => {
      ...
      expect(result.steps.indexOf('draft')).toBeLessThan(
        result.steps.indexOf('publish')
      );
  ...
  describe('onWorkInProgress', () => {
    ...
    it('should propagate an error from onWorkInProgress', async () => {
      ...
      await expect(
        jsonSurgery(createClient(), original, 'Increase the price by 10%', {
          onWorkInProgress,
        })
  ...
  describe('bulk operations', () => {
    ...
    it('should append a copy of a nested object in a list in 2 or fewer iterations', async () => {
      ...
      expect(result.sesame_street_characters[0]).toEqual(
        original.sesame_street_characters[0]
      );

      const expectBert = JSON.parse(
        JSON.stringify(original.sesame_street_characters[0])
      );
  ...
  describe('giveUp limits', () => {
    it('should give up after iteration limit', async () => {
      ...
      try {
        ...
        const result = (error as JSONSurgeryError).obj;
      ...
      throw new Error(
        'Expected jsonSurgery to throw after reaching iteration limit.'
      );
    }, 180000);
    ...
    it('should give up after time limit', async () => {
      ...
      throw new Error(
        'Expected jsonSurgery to throw after reaching time limit.'
      );
    }, 180000);
  });
});
